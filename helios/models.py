...
class Election(HeliosModel):
    ...
    ELECTION_TYPES = (
        ('election', ugettext_lazy('Election')),
        ('referendum', ugettext_lazy('Referendum'))
    )

    # dummy dictionary with translations
    ELECTION_TYPES_FORMS = {
        'election': (
            pgettext_lazy('nominative', 'election'),
            pgettext_lazy('locative', 'election'),
            pgettext_lazy('election', 'private'),
            pgettext_lazy('election', 'public'),
            pgettext_lazy('election', 'is'),
            pgettext_lazy('election', 'is <u>not</u>'),
            pgettext_lazy('election', 'was initially scheduled to end'),
            pgettext_lazy('election', 'but has been extended'),
            pgettext_lazy('election', 'is scheduled to end'),
            pgettext_lazy('election', 'ends'),
            pgettext_lazy('election', 'this'),
            pgettext_lazy('election', 'This'),
        ),
        'referendum': (
            pgettext_lazy('nominative', 'referendum'),
            pgettext_lazy('locative', 'referendum'),
            pgettext_lazy('referendum', 'private'),
            pgettext_lazy('referendum', 'public'),
            pgettext_lazy('referendum', 'is'),
            pgettext_lazy('referendum', 'is <u>not</u>'),
            pgettext_lazy('referendum', 'was initially scheduled to end'),
            pgettext_lazy('referendum', 'but has been extended'),
            pgettext_lazy('referendum', 'is scheduled to end'),
            pgettext_lazy('referendum', 'ends'),
            pgettext_lazy('referendum', 'this'),
            pgettext_lazy('referendum', 'This'),
        )
    }
    ...
    def eligibility_constraint_for(self, user_type):
        ...
        relevant_constraints = [constraint['constraint'] for constraint in self.eligibility if
                                constraint['auth_system'] == user_type and 'constraint' in constraint]
    ...
    def pretty_eligibility(self):
        if not self.eligibility:
            return ugettext_lazy("Anyone can vote.")
        else:
            return_val = "<ul>"
            ...
            for constraint in self.eligibility:
                if 'constraint' in constraint:
                    ...
                    return_val += "<li> " + ugettext_lazy('any %s user') % constraint['auth_system'] + "</li>"
    ...
    def issues_before_freeze(self):
        ...
        if self.questions == None or len(self.questions) == 0:
            action = ugettext_lazy('Add question to the ballot')
            issues.append(
                {'type': 'questions',
                 'action': action}
            )
        ...
        if len(trustees) == 0:
            action = ugettext_lazy('Add at least one truestee')
            issues.append({
                'type': 'trustees',
                'action': action
            })
        ...
        for t in trustees:
            if t.public_key == None:
                action = ugettext_lazy('have trustee %s generate a keypair') % t.name
                issues.append({
        ...
        if self.voter_set.count() == 0 and not self.openreg:
            action = ugettext_lazy('enter your voter list (or open registration to the public)')
            issues.append({
                "type": "voters",
                "action": action
            })
    ...
    def winners(self):
        ...
        """
        return [self.one_question_winner(self.questions[i], self.result[i], self.num_cast_votes) for i in
...
class ElectionLog(models.Model):
    ...
    FROZEN = ugettext_lazy("frozen")
    VOTER_FILE_ADDED = ugettext_lazy("voter file added")
    DECRYPTIONS_COMBINED = ugettext_lazy("decryptions combined")
