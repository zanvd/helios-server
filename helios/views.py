...
import csv, urllib.request, urllib.parse, urllib.error, os, base64
...
def encrypt_ballot(request, election):
  ...
  answers = utils.from_json(request.POST.get('answers_json', request.GET['answers_json']))
...
def password_voter_login(request, election):
  ...
  return_url = request.POST.get('return_url', request.GET.get('return_url', reverse(one_election_cast_confirm, args=[election.uuid])))
  ...
  login_url = request.POST.get('login_url', request.GET.get('login_url', None))
...
def voters_email(request, election):
  ...
  voter_id = request.POST.get('voter_id', request.GET.get('voter_id', None))
